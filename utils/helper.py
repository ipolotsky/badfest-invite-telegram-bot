...
def get_insta(text):
    pattern_link = re.compile(r'(https://|http://|)(www\.|)instagram\.com/([A-Za-z\d\-\_\.]+)', flags=re.IGNORECASE)
    pattern_username = re.compile(r'(@|)([A-Za-z0-9\-\_\.]+)')

    if re.search("^http", text) or re.search("^instagram\.com", text, re.IGNORECASE):
        try:
